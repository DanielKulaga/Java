public class MergeSort extends Thread {
    ...
    public static void test(int iloscWatkow, int[] tablica) throws InterruptedException {
        ...
        Watek[] watek = new Watek[iloscWatkow];
        for (int i = 0; i < iloscWatkow ; i++) {

            watek[i].run(poczatek,poczatek + przedzial  , tablica);
            poczatek = poczatek + przedzial + 1;

        }
        mergesort(poczatek, tablica.length - 1, tablica); //glowny watek sortuje koncowke tablicy
        for (int i = 0; i < iloscWatkow ; i++) {
    ...
    /*@Override
    public void run() {
        mergesort(lewa, prawa, tablica);
    }*/
    static class Watek extends Thread {
        void run(int lewa, int prawa, int[] tablica) {
